...
class Kiwi {
     ...
     * @param bool $routeEnv = false If you set this to true, it will be converted to a list for route searching.
     * @return string load env data
     */
    public static function loadEnv(string $filePath, bool $routeEnv = false) : array {
        ...
        while (($line = fgets($fs))) {
            ...
            if ($routeEnv) {
                $content = [];
                $routeBuff = explode("," , $buff[1]);
                foreach ($routeBuff as $rb_) {
                    $rb_ = trim($rb_);
                    $rb2_ = explode(":", $rb_);

                    $content[trim($rb2_[0])] = trim($rb2_[1]);
                }
                $contents[trim($buff[0])] = $content;
            }
            else{
                $contents[trim($buff[0])] = trim($buff[1]);
            }
        }
        fclose($fs);
     ...
     * @param bool Execution result
     */
    public static function saveEnv(string $filePath, array $envData, bool $routeEnv = false) : bool {
    ...
    public static function composerAutoload() : void {
        require KIWI_ROOT_CONTAINER . "/" . Routes::$route -> container ."/app/vendor/autoload.php";
    }   
}